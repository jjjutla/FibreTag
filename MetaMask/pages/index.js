...
function Home({ chains }) {
  ...
  const { testnets, testnet, search: rawSearch } = router.query;

  // Set default value for search
  const search = !rawSearch || rawSearch === "" ? "vechain" : rawSearch;
      ...
      <Layout>
        <HypeLabContext client={client}>
          <React.Suspense fallback={<div className="h-screen"></div>}>
            <div className="dark:text-[#B3B3B3] text-black grid gap-0 grid-cols-1 place-content-between pb-4 sm:pb-10 sm:grid-cols-[repeat(auto-fit,_calc(50%_-_15px))] 3xl:grid-cols-[repeat(auto-fit,_calc(33%_-_20px))] isolate grid-flow-dense">
