        ...
        <Spin spinning={loading}>
          {!tokens.length &&
            <div style={{ textAlign: 'center' }}>
              <Paragraph>
                <Link to={`https://${process.env.CHARGEBEE_HOSTED_ID}.chargebee.com/hosted_pages/checkout?payment_items[item_price_id][0]=${process.env.CHARGEBEE_payment_ID}&payment[cf_wallet_address]=${account}`}>
                  <Button size='large' type="primary">Buy Item</Button>
                </Link>
              </Paragraph>
              <Paragraph>
                use <Text code>4111 1111 1111 1111</Text> as  test card number.
              </Paragraph>
            </div>
          }
              ...
              {tokens.map(({ tokenId, owner }) => (
                <Col span={24} key={tokenId}>
                  <Descriptions column={1} labelStyle={{ minWidth: 148 }} size='small'>
                    <Descriptions.Item label="payment">{tokenId.slice(0, 8)}…{tokenId.slice(-8)}</Descriptions.Item>
